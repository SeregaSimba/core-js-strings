...
function countVowels(str) {
  ...
  str.split('').forEach((strok) => {
    if (vowels.includes(strok)) i += 1;
  });
...
function isPalindrome(str) {
  const strit = str.toLowerCase().replace(/[\W_]/g, '');
  const revers = strit.split('').reverse().join('');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < strSplit.length; i += 1) {
    if (strSplit[i].length > str.length) {
      str = strSplit[i];
    }
  }
  return str;
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((result) => result.split('').reverse().join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  ...
  const alfCode13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    const cod = alf.indexOf(str[i]);

    if (cod === -1) result += str[i];
    else result += alfCode13[cod];
  }
  return result;
}
...
function getCardId(value) {
  ...
  return cards.findIndex((arr) => arr === value);
}
